...
with fiona.open(shp_countries) as c:
    ...
    for feature in c:
        country_names = feature['properties']['NAME']
        features = filter_features_geom(shp_gdal, {"mask":feature["geometry"]})
        print country_names+' : '+ str(len(features[1]))



	        # Aus countries ueber Attribut Deutschland finden
'''	        
	        if filter_features_properties(feature, {"prop":"NAME", "value":cname}):
